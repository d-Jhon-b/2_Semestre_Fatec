...
def mostrar_usuarios(request: Request):
    ...
    return templates.TemplateResponse("index.hmtl", {"request": request, "usuarios":usuarios })

def mostrar_edicao(request: Request, user_id):
    usuario = user_model.buscar_usuario_por_id(user_id)
    usuarios = user_model.listar_usuarios()
    return templates.TemplateResponse("usuarios/editar.html",{"request:": request,"usuario_editar": usuario, "usuarios": usuarios})
...
async def atualizar_usuarios(request: Request, user_id: int, nome:str = Form(...), email:str = Form(...)):
    user_model.excluir_usuario(user_id, nome, email)
    return RedirectResponse("/", status_code=201)
